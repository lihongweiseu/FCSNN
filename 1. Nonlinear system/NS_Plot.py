...
cm = 1 / 2.54
model_name=['FCSNN (6)','CSNN (2)','LSTM (20)','PhyCNN']
...
ax.plot([N1+0.5, N1+0.5], [0.5, 1.015], 'k', lw=0.5)
for i in range(4):
    ax.scatter(N, PCC[:,i], c=colors[i], marker=markers[i], s=10, alpha=0.8, linewidths=0, label=model_name[i]) #
...
ax.text(5, 0.55, 'Training', ha='center', va='center', fontsize=7)
ax.text(55, 0.55, 'Testing', ha='center', va='center', fontsize=7)
...
legend = ax.legend(loc='lower right', bbox_to_anchor=(0.9, 0.08), borderpad=0.3, borderaxespad=0, handlelength=1.8,
                   edgecolor='black', fontsize=7, ncol=1, columnspacing=0.2, handletextpad=0.0, markerscale=1.5)  # labelspacing=0
...
# fig1.savefig(fname = 'F_NS_PCC.pdf', format = 'pdf')
#%%
j=[5,92]
yl= [[60, 30],[30, 15]]
xl = [[0, 30, 5],[0, 30, 5]]
...
for i in range(2):
    ...
    ax.plot(t, y_ref[:,j[i]-1:j[i]]*100, color='k', lw=0.5, label='Ground truth')
    ax.plot(t, y_pre[0,:,j[i]-1:j[i]]*100, color='r', dashes=[8, 2, 4, 2], lw=0.5, label='FCSNN (6)')
    ...
    ax.text(15, yl[i][1]*1.6, 'Case '+str(j[i])+', PCC: '+ '{:.5f}'.format(PCC[j[i]-1,0].item()), ha='center', va='center', fontsize=7)
    ax.set_ylabel(r'Disp. (cm)', fontsize=7, labelpad=-1)
    labels = [item.get_text() for item in ax.get_xticklabels()]
    labels_new = [i.replace('$\\mathdefault{', '').replace('}$', '') for i in labels]
    ax.set_xticklabels(labels_new)
    labels = [item.get_text() for item in ax.get_yticklabels()]
...
# fig2.savefig(fname = 'F_NS_pre.pdf', format = 'pdf')