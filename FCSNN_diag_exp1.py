...
def FCSNN(in_s, state_s, out_s, non_layer_s, non_neuron, A_diag0, dt, device, bias_status):
    # Define FCSNN
    class FCSNN(nn.Module):
        ...
        def forward(self, u):
            ...
            A_diag = torch.matmul(-torch.square(A_diag0.to(device)),torch.triu(torch.ones(state_s, state_s).to(device), diagonal=0))
...
def FCSNN_training(dt, u, FCSNN_model, N, y_ref, in_s, state_s, out_s, non_layer_s, non_neuron, A_diag0, device, bias_status):
    ...
    loss_m = loss.item()
    FCSNN_model_m = FCSNN(in_s, state_s, out_s, non_layer_s, non_neuron, A_diag0, dt, 'cpu', bias_status)
