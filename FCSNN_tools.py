...
def FCSNN(in_s, state_s, out_s, non_layer_s, non_neuron, A_ini, B_ini, dt, device, bias_status):
    # Define FCSNN
...
def FCSNN_training(dt, u, FCSNN_model, N, y_ref, in_s, state_s, out_s, non_layer_s, non_neuron, A_ini, B_ini, device, bias_status):
    u_torch = torch.tensor(u, dtype=torch.float).to(device)
    y_ref_torch = torch.tensor(y_ref, dtype=torch.float).to(device)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(FCSNN_model.parameters(), 1e-2) # , weight_decay=1e-8, amsgrad=True
    ...
    loss_m = loss.item()
    FCSNN_model_m = FCSNN(in_s, state_s, out_s, non_layer_s, non_neuron, A_ini, B_ini, dt, 'cpu', bias_status)
