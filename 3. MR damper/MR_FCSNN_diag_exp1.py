...
from FCSNN_diag_exp1 import FCSNN, FCSNN_training
...
for i in range(7, 8):
    ...
    FCSNN_model = FCSNN(in_s, state_s, out_s, non_layer_s, non_neuron, A_diag0, dt, device, bias_status)
    ...
    FCSNN_model_m, loss_all, loss_m, cost_time, im, cost_time_str = FCSNN_training(dt, u[0:Nt1,:,:], FCSNN_model, N, y_ref[0:Nt1,:,:], in_s, state_s, out_s, non_layer_s, non_neuron, A_diag0, device, bias_status)
    torch.save(FCSNN_model_m.state_dict(), './saved_models/'+name+'_FCSNN_diag_exp1_'+str(state_s)+'.pt')
...
state_s=7
non_neuron = state_s * np.ones(non_layer_s, dtype=np.int32) + in_s
FCSNN_model = FCSNN(in_s, state_s, out_s, non_layer_s, non_neuron, A_diag0, dt, 'cpu', bias_status)
pt_match = glob.glob(os.path.join('./saved_models', '*_FCSNN_diag_exp1_'+str(state_s)+'.pt'))
