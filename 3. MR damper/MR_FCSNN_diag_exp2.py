...
from FCSNN_diag_exp2 import FCSNN, FCSNN_training
...
for i in range(7, 8):
    ...
    FCSNN_model = FCSNN(in_s, state_s, out_s, non_layer_s, non_neuron, Para_diag, dt, device, bias_status)
    ...
    FCSNN_model_m, loss_all, loss_m, cost_time, im, cost_time_str = FCSNN_training(dt, u[0:Nt2,:,:], FCSNN_model, N, y_ref[0:Nt2,:,:], in_s, state_s, out_s, non_layer_s, non_neuron, Para_diag, device, bias_status)
    torch.save(FCSNN_model_m.state_dict(), './saved_models/'+name+'_FCSNN_diag_exp2_'+str(state_s)+'.pt')
...
non_neuron = state_s * np.ones(non_layer_s, dtype=np.int32) + in_s
FCSNN_model = FCSNN(in_s, state_s, out_s, non_layer_s, non_neuron, Para_diag, dt, 'cpu', bias_status)
pt_match = glob.glob(os.path.join('./saved_models', '*_FCSNN_diag_exp2_'+str(state_s)+'*.pt'))
# # pt_match = glob.glob(os.path.join('./saved_models', 'MR_FCSNN_diag_20_9.9996e1_19976_2h 7m 34.746s.pt'))
